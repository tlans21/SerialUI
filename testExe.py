...
ui_file_path = os.path.join(script_dir, "My_gui_test.ui")
...
class WindowClass(QMainWindow, form_class) :
    ...
    def openPort(self):  
            ...
            while True:    
                if self.ser and self.ser.readable():
                    try:
                        ...
                        self.buffer = hex_representation.split()
                        ...
                        if self.ser.in_waiting <= self.byteLength:
                            ...
                            hex_representation = ' '.join([format(byte, '02X') for byte in res])
                            self.buffer.extend(hex_representation.split())
        ...
        self.threadInstance = threading.Thread(target=readThread)
    ...
    def inputBtn_Push(self):
        ...
        self.period = float(interval) * 1000   # ms 단위 이므로 1000울 곱합니다.
    ...
    def inputBtn_2_Push(self):
        ...
        if not interval_2 or not interval_2.replace('.', '', 1).isdigit():
            ...
            return
        #print(interval)2)
        self.period_2 = float(interval_2) * 1000 # ms 단위 
